...
PROJECT_ROOT="$(cd "${HELPER_SCRIPT_DIR}/.." &> /dev/null && pwd)" # Project root is one level up
...
if [ ! -f "$SOURCE_PATH" ]; then
  ...
  echo "Please ensure the project has been built ('./u7 build' or './u7 build --release')." >&2
