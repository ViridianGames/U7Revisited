...
void ObjectEditorState::Init(const string& configfile)
{
	...
	m_gui->m_Scale = g_DrawScale;
	m_gui->m_Pos = Point(240 * g_DrawScale, 0);
	m_gui->SetLayout(240 * g_DrawScale, 0, 80, 180, Gui::GUIP_UPPERRIGHT);
...
void ObjectEditorState::Update()
{
	...
	if (g_Input->WasKeyPressed(KEY_a) || m_gui->GetActiveElementID() == GE_PREVSHAPEBUTTON)
	{
	...
	if (g_Input->WasKeyPressed(KEY_d) || m_gui->GetActiveElementID() == GE_NEXTSHAPEBUTTON)
	{
		int newShape = m_currentShape + 1;
	...
	if (g_Input->WasKeyPressed(KEY_w) || m_gui->GetActiveElementID() == GE_NEXTFRAMEBUTTON)
	{
	...
	if (g_Input->WasKeyPressed(KEY_s) || m_gui->GetActiveElementID() == GE_PREVFRAMEBUTTON)
	{
		int newFrame = m_currentFrame - 1;
		if (newFrame < 0)
		{
	...
	if (g_Input->WasKeyPressed(KEY_SPACE) || m_gui->GetActiveElementID() == GE_NEXTDRAWTYPEBUTTON)
	{
