...
void U7Object::Init(const string& configfile, int unitType, int frame)
{
   ...
   if (m_drawType == ObjectDrawTypes::OBJECT_DRAW_CUBOID)
   {
      if (objectData->m_mesh == nullptr)
      {
         ...
         float   slantX = 1 - offset;
         float   slantY = 1 - offset;
         ...
         {
            Texture* workingTexture = g_shapeTable[unitType][theseFrames];
...
void U7Object::Draw()
{
   ...
   if (m_Visible)
   {
      ...
      ObjectDrawTypes type = std::get<0>(g_ObjectTypes.at(m_ObjectType));
      switch (type)
      {
      ...
      case ObjectDrawTypes::OBJECT_DRAW_CROSSBILLBOARD:
