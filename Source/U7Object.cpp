...
void U7Object::SetPos(Vector3 pos)
{
	...
	int shapeIdx = m_shapeData->GetShape();
	int frameIdx = m_shapeData->GetFrame();
	...
	{
			float txWide = float(m_shapeData->m_originalTexture->width);
			float txHigh = float(m_shapeData->m_originalTexture->height);
			if (shapeIdx == 911 && frameIdx == 10)
			{
				//printf("	shape %d frame %d texture %f %f\n", shapeIdx, frameIdx, txWide, txHigh);
				//txWide -= 32.0;
				//nudgeWide = 8.0 / 8.0f;
			}
			dims = Vector3{ txWide / 8.0f, 0.0f, txHigh / 8.0f };
			boundingBoxAnchorPoint = Vector3Add(m_Pos, Vector3{ -dims.x + 1, 0, -dims.z + 1 });
	}
	else if (m_drawType == ShapeDrawType::OBJECT_DRAW_TERRAIN)
	{}
	else if (m_drawType == ShapeDrawType::OBJECT_DRAW_OCEAN)
	{}
	else if (m_drawType == ShapeDrawType::OBJECT_DRAW_RIVER)
	{}
	else
	{
			dims = Vector3{ objectData->m_width, objectData->m_height, objectData->m_depth };
