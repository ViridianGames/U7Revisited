...
void LoadingState::UpdateLoading()
{
	if (!m_loadingFailed)
	{
		if (!m_loadingVersion)
		{
			...
			printf("  UpdateLoading: Loading version ...\n");
			LoadVersion();
		...
		if (!m_loadingChunks)
		{
			...
			printf("  UpdateLoading: Loading chunks ...\n");
			LoadChunks();
		...
		if (!m_loadingMap)
		{
			...
			printf("  UpdateLoading: Loading mapfile ...\n");
			LoadMap();
			m_loadingMap = true;
			return;
		}
		...
		if (!m_loadingObjects)
		{
			...
			printf("  UpdateLoading: Loading objects ...\n");
			CreateObjectTable();
		...
		if(!m_loadingModels)
		{
			...
			printf("  UpdateLoading: Loading models ...\n");
			LoadModels();
		...
		if (!m_loadingShapes)
		{
			...
			printf("  UpdateLoading: Loading shapes ...\n");
			CreateShapeTable();
			m_loadingShapes = true;
			printf("  UpdateLoading: Loading shapes ... finished\n");
			return;
		}
		...
		if (!m_makingMap)
		{
			...
			printf("  UpdateLoading: Making map ...\n");
			MakeMap();
			m_makingMap = true;
			return;
		}
		...
		if (!m_loadingIFIX)
		{
			...
			printf("  UpdateLoading: Loading IFIX ...\n");
			LoadIFIX();
			m_loadingIFIX = true;
			return;
		}
		...
		if (!m_loadingIREG)
		{
			...
			printf("  UpdateLoading: Loading IREG ...\n");
			LoadIREG();
			m_loadingIREG = true;
			return;
		}
		...
		if (!m_loadingInitialGameState)
		{
			...
			printf("  UpdateLoading: Initializing game state ...\n");
	...
	printf("  UpdateLoading: Transitioning to title state ...\n");
				...
				for (int chunkx = 0; chunkx < 16; ++chunkx)
				{
					...
					else
					{
						...
						for (int w = 0; w < (thisentry.length / 2); w += 2)
						{
							...
							//printf("  LoadIFIX chunk(%d %d) calling AddObject\n", chunkx, chunky);
							// turned off AddObject for IFIX
		...
		for (int j = 0; j < 192; ++j)
		{
			...
			for (int k = 0; k < 16; ++k)
			{
				for (int l = 0; l < 16; ++l)
				{
					...
					{
						//printf("  LoadMapMap shape reference %d\n", shapenum);
					}
				}
			}
		}
...
int LoadingState::IREGItem(bool printDebug, int scPos[2], int containerDepth, int dataLen, unsigned char* data)
{
	...
	if (dataLen == 6)
	{
		...
		if (isEgg == false	&& worldObject == true) //  Eggs
		{
			...
			{
				if (shapeIdx == interestedShape)
				{
					//printf("  LoadMakeMap chunk(%d %d) calling AddObject Animated %d %d\n", chunkx, chunky, shapeIdx, frameIdx);
				}
...
void LoadingState::CreateObjectTable()
{
	...
	for (int i = 0; i < 1024; ++i)
	{
		...
		//printf("shape %d	weight %f	volume %f\n", i, g_objectTable[i].m_weight, g_objectTable[i].m_volume);
