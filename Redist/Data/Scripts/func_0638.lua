...
function func_0638(eventid, itemref)
    ...
    if eventid == 2 then
        if local0 == 144 and get_item_type(itemref) == 642 then
            add_dialogue("@Odd. The page is smudged with dirt here. I cannot make out this text.@")
        ...
        if switch_talk_to(itemref, -356) then
            if not get_flag(805) then
                set_flag(805, true)
                add_dialogue("@Why, a page has fallen out of the book!@")
            end
        end
    end
    ...
    elseif local0 == 136 then
        ...
        add_dialogue("     The absence of Control, Passion and Diligence is Chaos.")
        add_dialogue("     The three principles, when mastered, lead to the Eighth Virtue, which is Singularity.")
        add_dialogue("     A circle has no end. It continues forever, turning upon itself in a continuous, endless cycle.")
        add_dialogue("     Our society is a circle, continuing forever, turning upon itself in an endless cycle. We are the circle, and the circle is us.")
    elseif local0 == 137 then
        add_dialogue("~~ ~~CHICKEN RAISING FOR FUN AND PROFIT~~ ~~by Lady Leigh*")
        add_dialogue("     While handling poultry might appear to be a simple task, I assure thee that such is not the case. As with any other creature, thou must be aware of several important factors when raising chickens, or any other type of fowl.")
        add_dialogue("     In addition, thou must be aware of the best methods of feeding thy flock, and what to do when they cease laying eggs. In fact, one of the more delicate operations is the proper removal of the chicken's neck feathers, shouldst thou decide to use the creature for a meal.")
    elseif local0 == 138 then
        add_dialogue("~~ ~~THE ART OF FIELD DRESSING~~ ~~by Lady Leigh*")
        add_dialogue("     Though I personally find the entire concept utterly distasteful, I have oft been asked how one should go about preparing a recently-slain creature for the table. Therefore, I have consented to describe the process, though I must warn thee that the details are not for the weak of stomach.")
        add_dialogue("-- Lady Leigh of Serpent's Hold")
        add_dialogue("     As long as one is careful to keep the knife away from the intestines, one should have little difficulty in preparing the meat for cooking. Be especially careful around the heart, for there is a small gland located nearby that, if pierced, will ruin the meat.")
    elseif local0 == 139 then
        add_dialogue("~~ ~~WATER ON THE ROCKS~~ ~~by Felcrodan*")
        add_dialogue("     (This text is heavily water damaged and mostly unreadable. Only a few sections remain that are still legible.)")
        add_dialogue("     The using of sextants is no longer necessary for finding one's way around Britannia, for the recent advances in cartography have provided us with maps of the entire continent. However, there are still some mariners who prefer the old ways, plotting their courses by the stars and using their trusty sextants to pinpoint their locations.")
    elseif local0 == 140 then
        add_dialogue("~~ ~~A SHORT TREATISE ON THE HIERARCHY OF THE FELLOWSHIP~~ ~~by Batlin*")
        add_dialogue("     The hierarchy of The Fellowship is quite simple. At the top is the voice of the Guardian. Below him is myself, the head of The Fellowship, and my two assistants, Elizabeth and Abraham.")
        add_dialogue("     All citizens of Britannia are welcome to join the ranks of The Fellowship as brothers and sisters. One must merely express a desire to join, and then prove one's worth by accomplishing the directives that are given by me or my assistants.")
        add_dialogue("     Members of The Fellowship are expected to live by the philosophy of the Triad of Inner Strength, and to spread the message of The Fellowship to others.")
        add_dialogue("     Just below the brothers and sisters are the candidates for membership, those who have expressed a desire to join, but have yet to prove their worth.")
        add_dialogue("     Much of the financing for the various activities of The Fellowship comes from the generous donations of its members. Those who are able are expected to tithe a portion of their income to the cause.")
    elseif local0 == 141 then
        add_dialogue("~~ ~~A BAKER'S HANDBOOK~~ ~~by F. Frederickson*")
        add_dialogue("     The fundamental ingredient in any baker's arsenal is flour. Without it, thou hast no chance of creating even the simplest of breads. Fortunately, flour is quite easy to come by, for any miller will sell it to thee by the sack.")
        add_dialogue("     It is important to realize that not all flours are created equal. The best flour comes from the mills of Minoc, for they use a special process that removes the chaff and other impurities from the grain before it is ground.")
        add_dialogue("     To the four, thou must add water, and perhaps a pinch of salt for taste. Some bakers also add a bit of yeast, but this is not necessary if thou art in a hurry. Mix the ingredients together in a bowl, and then knead the dough until it is smooth and elastic. Once this is done, place it near the hearth and bake it. After but a few minutes, thou wilt have quite tasty bread to eat.")
    ...
    elseif local0 == 144 then
        ...
        add_dialogue("     However, finding the Stone is only half the battle, for now thou must perform magicks beyonds the abilities of normal men. With a ... thou must.........")
