...
function utility_event_0522(eventid, objectref)
    ...
    if eventid ~= 3 then
        if eventid == 2 then
            if get_object_shape(objectref) == 154 then
                var_001A = {get_object_position(objectref), 200, 6}
                var_0003 = find_nearby(16, 20, 275, var_001A)
                if not var_0003 then
                    for var_001B in ipairs(var_0003) do
                        var_001A = {get_object_position(var_0001), -359, 0}
                        var_001D = find_nearby(16, 0, 275, var_001A)
                        for var_001E in ipairs(var_001D) do
                            var_0002 = execute_usecode_array(var_0020, {17480, 7724})
                            var_0002 = execute_usecode_array(get_npc_name(-356), {1802, 17493, 17452, 7715})
                        end
                    end
                end
            elseif get_object_shape(objectref) == 721 or get_object_shape(objectref) == 989 then
                var_0001 = find_nearby(8, 30, 354, get_npc_name(-356))
                if not var_0001 then
                    var_0021 = get_object_position(var_0001)
                    sprite_effect(-1, 0, 0, 0, var_0021[2] - 2, var_0021[1] - 2, 8)
                    sprite_effect(-1, 0, 0, 0, var_0021[2] - 2, var_0021[1] - 2, 7)
                    play_sound_effect(52)
                    get_object_frame(var_0001, 19)
                    var_0002 = execute_usecode_array(var_0001, {1802, 8021, 5, 7975, 4, 7769})
                else
                    var_001A = {get_object_position(var_0001), -359, 6}
                    var_0003 = find_nearby(16, 0, 275, var_0001)
                    for var_0022 in ipairs(var_0003) do
                        var_0002 = execute_usecode_array(var_0001, {17480, 7724})
                        var_0002 = execute_usecode_array(get_npc_name(-356), {1802, 17493, 17452, 7715})
                    end
                end
            end
            if not get_item_flag(-356, 4) then
                abort()
            end
            if get_object_shape(objectref) == 354 then
                var_0024 = get_distance(get_npc_name(-356), objectref)
                if var_0024 < 20 and not get_item_flag(get_npc_name(-356), 23) then
                    var_0021 = get_object_position(objectref)
                    sprite_effect(-1, 0, 0, 0, var_0021[2] + 3, var_0021[1] + 3, 17)
                    ...
                    for var_0026 in ipairs(var_0025) do
                        ...
                        halt_scheduled(var_0028)
                        var_0002 = execute_usecode_array(var_0028, var_0029)
                    end
                    earthquake(3)
                    var_002D = random2(20, 5)
                    var_0002 = execute_usecode_array(objectref, {1802, 8021, 1, 17447, 8047, 1, 17447, 8044, 2, 17447, 8045, 1, 17447, 8556, var_002D, 17447, 7780})
                else
                    var_002D = random2(15, 5)
                    var_0002 = execute_usecode_array(objectref, {1802, 8021, 10, 8487, var_002D, 17447, 7780})
                end
            end
        end
        return
    ...
    if not _get_object_quality(objectref) and not get_flag(827) then
        ...
        if not var_0000 then
            ...
            var_0002 = execute_usecode_array(var_0000, {8033, 2, 17447, 8044, 1802, 17493, 7937, 3, 17447, 8045, 1, 17447, 8044, 2, 17447, 8033, 2, 17447, 8048, 3, 17447, 7791})
            var_0003 = find_nearby(0, 20, 336, objectref)
            for var_0004 in ipairs(var_0003) do
                if get_object_frame(var_0001) == 7 or get_object_frame(var_0001) == 9 then
                    var_0006 = create_new_object(338)
                    get_object_frame(var_0006, get_object_frame(var_0001))
                    var_0002 = update_last_created(get_object_position(var_0001))
                    remove_item(var_0001)
                end
            end
            var_0003 = find_nearby(0, 20, 338, objectref)
            ...
            var_0003 = find_nearby(0, 30, 168, objectref)
