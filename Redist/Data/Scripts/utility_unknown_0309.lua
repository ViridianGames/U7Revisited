...
function utility_unknown_0309(eventid, objectref)
    local var_0000, var_0001, var_0002, var_0003, var_0004, var_0005

    start_conversation()
    if eventid == 3 then
        var_0000 = find_nearby(0, 0, 658, objectref) --- Guess: Sets NPC location
        if var_0000 then
    ...
    elseif eventid == 2 then
        var_0002 = get_object_position(objectref) --- Guess: Gets position data
        var_0003 = find_nearby(0, 2, 831, objectref) --- Guess: Sets NPC location
        if #var_0003 > 0 then
            remove_item(objectref) --- Guess: Unknown object operation
            var_0004 = get_object_status(377) --- Guess: Gets item status
            if var_0004 then
                set_object_flag(var_0004, 18)
                set_object_frame(objectref, 0)
                var_0001 = update_last_created(var_0002) --- Guess: Updates position
                var_0005 = random(1, 3)
                if var_0005 == 1 then
