...
function func_0710(eventid, itemref)
    ...
    if eventid == 1 then
        ...
        var_0000 = unknown_0035H(8, 40, 1015, itemref) --- Guess: Sets NPC location
        var_0001 = false
        var_0002 = false
        ...
        if get_flag(795) and not get_flag(796) then
            bark(itemref, "@All the golems are alive now.@")
            add_dialogue("*")
            return
        end
        var_0006 = false
        ...
        if not var_0007 then
            bark(itemref, "@The golem must be centered in the pentacle of stones.@")
        else
            ...
            for i = 1, 5 do
                ...
                var_000C = get_item_frame(var_000B) --- Guess: Gets item frame
            ...
            if not var_0006 then
                ...
                if var_0008 > 0 and var_0008 < 5 then
                    bark(itemref, "@Only " .. var_0008 .. var_000D .. " covered.@")
                else
                    bark(itemref, "@Blood must cover the rocks.@")
                end
            end
        end
        set_item_behavior(14, itemref) --- Guess: Sets item behavior
        trigger_item_effect(itemref) --- Guess: Triggers item effect
        ...
        if var_0006 and var_0007 then
            var_000F = add_container_items(356, {1812, 17493, 7715})
        end
    elseif eventid == 2 then
        set_flag(796, true)
        var_0010 = unknown_0018H(itemref) --- Guess: Gets position data
        calle_08E6H(itemref) --- External call to activate object
        var_0011 = get_item_status(414) --- Guess: Gets item status
        set_item_frame(var_0011, 5) --- Guess: Sets item frame
        var_0012 = unknown_0026H(var_0010) --- Guess: Updates position
        var_0013 = get_item_status(797) --- Guess: Gets item status
        set_item_quality(var_0013, 244) --- Guess: Sets item quality
        set_item_frame(var_0013, 4) --- Guess: Sets item frame
        var_0012 = get_item_position(var_0011) --- Guess: Gets item position
        var_0014 = get_item_status(203) --- Guess: Gets item status
        set_item_frame(var_0014, 10) --- Guess: Sets item frame
        var_0010[1] = var_0010[1] + 4
        var_0010[2] = var_0010[2] - 1
        var_0010[3] = var_0010[3] + 2
        var_0012 = unknown_0026H(var_0010) --- Guess: Updates position
        bark(itemref, "@He gave up his heart... so that Adjhar may live!* Well, not to be morbid, but I suppose the incantation should work now.@")
