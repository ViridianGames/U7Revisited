...
function func_048D(eventid, itemref)
    ...
    if eventid == 1 then
        ...
        if not get_flag(453) then
            ...
            if local6 then
                switch_talk_to(3, 0)
                add_dialogue("Shamino steps near you and speaks in a whispered tone.~~\"Do not trust this one, " .. local1 .. ". Methinks he'll cause naught but evil.\"")
                hide_npc(3)
                switch_talk_to(141, 0)
            elseif local7 then
                switch_talk_to(1, 0)
                add_dialogue("Iolo steps near you and speaks in a whispered tone.~~\"Do not trust this one, " .. local1 .. ". Methinks he'll cause naught but evil.\"")
                hide_npc(1)
                switch_talk_to(141, 0)
            end
            ...
            if local8 then
                switch_talk_to(2, 0)
                add_dialogue("\"Uh, " .. local1 .. "? I am ready to go now,\" he says to you, cowering from the undead creature.*")
                hide_npc(2)
                switch_talk_to(141, 0)
            end
            set_flag(453, true)
        ...
        while true do
            ...
            elseif answer == "New Order" then
                add_dialogue("An expression of zeal lights the dead face of the Liche.~~\"Yes, " .. local0 .. ". The dead will rule! I will be their leader and thou canst become an Avatar... to ME!\"")
                remove_answer("New Order")
                save_answers()
                add_answer({"Fine!", "Over my dead body!"})
            elseif answer == "Over my dead body!" then
                add_dialogue("\"Why, " .. local0 .. ". I thought that was understood. It will be my pleasure to help thee enter the realm of the dead.\"")
                restore_answers()
            ...
            elseif answer == "other purposes" then
                if not get_flag(3) then
                    ...
                    add_dialogue("He gestures at the walls of the tower. \"It served as an effective barrier against the ravaging effects of the disrupted ether.\"")
                end
