...
function func_0948(eventid, itemref)
    ...
    local0 = 4
    local1 = 1
    add_dialogue(itemref, "\"Excellent! Dost thou have some flour for me?\"")
    local2 = get_answer()
    if local2 then
        add_dialogue(itemref, "\"Very good! Let me see how many sacks thou dost have...\"")
        local3 = check_condition(14, -359, 863, -357)
        local4 = check_condition(15, -359, 863, -357)
        if local3 == 0 or local4 == 0 then
            add_dialogue(itemref, "\"But thou dost not have a single one in thy possession! Art thou trying to trick me? Get out of my shoppe!\"")
            return
        end
        local5 = local3 + math.floor(local4 / local1) * local0
        add_dialogue(itemref, "\"Beautiful flour! " .. local3 .. "! That means I owe thee " .. local5 .. " gold. Here thou art! I shall take the flour from thee now!\"")
        ...
        add_dialogue(itemref, "\"No? Then thou art a -loaf-er! Ha ha ha!\"")
    end
    _RestoreAnswers()
    return
end