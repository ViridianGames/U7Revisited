...
function func_08F2(local0, local1)
    ...
    local3 = true
    local4 = call_08F1H("")
    add_dialogue("\"", local0, "! Thou ", local4, "!\"")
    local5 = {"life", "head", "blood"}
    local6 = local5[callis_0010(callis_005E(local5), 1)]
    add_dialogue("\"Shall I have my apology, or thy ", local6, "?\"")
    local7 = "Forgive me"
    ...
    while true do
        if cmp_strings(local7, 0x00A7) then
            local4 = call_08F1H("")
            add_dialogue("\"Forgive thee! What might I forgive in one such as thee, ", local4, "?\"")
            callis_0006(local8)
            callis_0006(local7)
        ...
        if cmp_strings("My lie", 0x010B) then
            ...
            add_dialogue("\"Of what lie speakest thou? Art thou not ", local0, "?\"")
            if call_090AH() then
                local4 = call_08F1H("")
                add_dialogue("\"Perhaps thou art not ", local0, ", for I have never seen the ", local4, ". Confess now thy true identity!\"")
                callis_0005(local1)
        ...
        if cmp_strings(local1, 0x0176) then
            ...
            add_dialogue("\"", local1, "! Perhaps honesty shall lift thee above the ", local4, " ", local0, "...\"")
            local2 = true
            break
        end
        break
    ...
    if not local2 then
        ...
        local9 = call_08F1H(local4)
        if local3 then
            add_dialogue("\"", local4, "! ", local9, "! Thy soul shall wail in the catacombs of the netherworld!\"")
            callis_001D(0, -10)
            callis_003D(2, -10)
        else
            add_dialogue("\"", local4, "! Fly from this place at once! I shall provide escort for thee with my bow. Return at thy peril, ", local9, ".\"")
            callis_001D(9, -10)
            callis_003D(0, -10)
        end
    else
