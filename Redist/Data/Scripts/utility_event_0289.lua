--- Party-member location banter (eggs near towns/dungeons/shrines).
--- var_0003 == 0 → bark only; == 1 → short dialogue.
...
function utility_event_0289(eventid, objectref)
    ...
    elseif var_0001 == 43 then
        ...
        -- Unknown quality — nothing to say
        return
    end

    if var_0000 == nil or var_0000 == "" then
        return
    end

    if var_0002 == 357 then
        var_0002 = get_player_id and get_player_id() or get_avatar_ref()
        if var_0002 == 356 or var_0002 == -356 or var_0002 == 0 then
            return
        end
    end

    if var_0003 == 0 then
        -- Bark only — no conversation UI
        if string.sub(var_0000, 1, 1) ~= "@" then
            var_0000 = "@" .. var_0000 .. "@"
        end
        if npc_id_in_party(var_0002) then
            bark(var_0002, var_0000)
        end
    else
        -- Short dialogue from a party member
        if npc_id_in_party(var_0002) then
            start_conversation()
