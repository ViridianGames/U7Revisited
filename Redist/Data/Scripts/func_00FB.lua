...
function func_00FB(eventid, itemref)
    ...
    if unknown_0058H(itemref) then
        var_0000 = check_flag_location(0, 199, 5, itemref)
        var_0001 = check_flag_location(0, 251, 5, itemref)
        if not is_in_int_array(itemref, var_0001) then
            var_0001 = var_0001 .. {itemref}
        end
        -- callis 0088, 2 (unmapped)
        if not unknown_0088H(10, 356) then
            var_0002 = get_object_quality(itemref)
