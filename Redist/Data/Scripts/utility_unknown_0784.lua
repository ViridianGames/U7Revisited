...
function utility_unknown_0784(eventid, objectref, arg1)
    ...
    var_0004 = find_nearby(16, 80, 275, var_0003) --- Guess: Sets NPC location
    var_0005 = 0
    ...
    for i = 1, 5 do
        ...
        if var_0000 == 1 then
            if var_0009 then
                var_000A = set_last_created(var_0009) --- Guess: Checks position
                update_last_created(358) --- Guess: Updates position
                play_sound_effect(10) --- Guess: Triggers event
            end
        else
            if var_0009 == 0 then
                var_000B = get_object_status(var_0001) --- Guess: Gets item status
                set_object_frame(var_000B, var_0002) --- Guess: Sets item frame
                var_000A = update_last_created(var_0003) --- Guess: Updates position
                var_000A = set_last_created(var_0009) --- Guess: Checks position
                update_last_created(358) --- Guess: Updates position
                play_sound_effect(83) --- Guess: Triggers event
            end
        end
    end
end