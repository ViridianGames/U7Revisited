...
function func_049B(eventid, itemref)
    ...
    if local3 then
        say("\"" .. local1 .. "? Thou, too, hast joined the evil organization? But how is this possible? Canst thou not see their tenets vie with the very virtues themselves? Dost thou not feel more like a sheep than a man? I am truly sorry, for if one of Britannia's greatest heroes has fallen in with such filth, then there is no hope for our great land!\" He turns away in disgust.*")
        return
    end
    ...
    if not get_flag(511) then
        ...
        say("Cubolt looks up. \"Yes, " .. local1 .. "?\"")
    end
    ...
    while true do
        ...
        elseif answer == "job" then
            say("\"I am a farmer, " .. local1 .. ". I manage my farm with the help of Tolemac, my brother, and Morz, our family friend.\"")
        ...
        elseif answer == "reconsider" then
            say("\"Unfortunately, Tolemac will not listen to me. However,\" he begins to smile hopefully, \"he just might listen to thee, " .. local1 .. ". Perhaps thou couldst talk him into reconverting. I would very much appreciate that! Perhaps,\" he adds, \"thou couldst also ask Morz not to join.\"")
            set_flag(470, true)
            set_flag(471, true)
            remove_answer("reconsider")
        elseif answer == "bye" then
            say("\"Take care, " .. local1 .. ".\"*")
            break
        end
    end
    return
