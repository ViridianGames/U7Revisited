...
function func_084F()
    ...
    if not local2 then
        switch_talk_to(1, 0)
        add_dialogue("\"Iolo whispers to you. \"Art thou quite certain, " .. local0 .. ", that thou dost wish to join with these people?\"")
    ...
    if not local13 and not local2 then
        switch_talk_to(1, 0)
        add_dialogue("You notice Iolo is whispering to Shamino. \"I do not think that " .. local0 .. " doth realize the significance of the situation. " .. local1 .. " cannot be dissuaded. Perhaps thou shouldst give it a try.\"")
        hide_npc(1)
        switch_talk_to(3, 0)
        add_dialogue("\"All right, I shall give it a try.\" He nudges you and whispers. \"Perhaps we should get out of here, " .. local1 .. ", before one of us does something they may later regret? Let us leave these premises, all right?\"")
    ...
    if not local13 then
        ...
        if not local15 then
            switch_talk_to(3, 0)
            add_dialogue("You hear Shamino desperately whispering to Dupre as they stand behind you. \"Dupre, we are having no success in showing the Avatar the mistake " .. local17 .. " must surely be making. Thou art our last hope.\"")
            hide_npc(3)
            switch_talk_to(4, 0)
