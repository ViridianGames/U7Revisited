...
function func_0491(eventid, itemref)
    ...
    if eventid == 1 then
        ...
        if local6 and not get_flag(446) then
            add_dialogue("Paulette perks up as she sees Rowena.~~\"Hello, milady. 'Tis good to see thee again. How art thou?\"*")
        ...
        if local7 and not get_flag(445) then
            ...
            add_dialogue("The mayor becomes quickly embarrassed as he tries to quiet the rather friendly Paulette.~~\"I, er, used to be a wine connoisseur of sorts,\" he says to you.*")
            hide_npc(147)
            switch_talk_to(145, 0)
        ...
        if not get_flag(458) then
            if not local1 then
                add_dialogue("You see a pretty, ghostly girl with long black hair. \"Hello, " .. local0 .. ". I am called Paulette. How may I help thee?\"")
            else
                add_dialogue("Standing before you, with a hand on her hip, is a lovely, young woman with long black hair. \"Ooooh... Thou art a big one, " .. local0 .. ".\" She traces the line of your bicep.~~\"I'd wager thou couldst lift me over thine head.\" She smiles enticingly. However, you doubt that you could even touch her in her ghostly state.~~\"Thou mayest call me Paulette, gorgeous. What can I do for thee?\" She winks at you.")
            end
            set_flag(458, true)
        else
            if not local1 then
                add_dialogue("\"Yes, " .. local0 .. "?,\" she asks sweetly.")
            else
        ...
        while true do
            ...
            if answer == "name" then
                add_dialogue("\"Why, " .. local0 .. ", hast thou forgotten already? I am Paulette.\"")
                remove_answer("name")
            elseif answer == "job" then
                if not local1 then
                    add_dialogue("\"Well, " .. local0 .. ", before the fire,\" she shudders, \"I used to be the barmaid here.\"")
                else
                    add_dialogue("\"Well, " .. local0 .. ". I used to clean tables here...\" As she says this, she bends over and pretends to wipe a table clean. You notice how low the cut of her bodice really is.~~\"...and serve people, like thyself. Of course, none so handsome.\" Her ghostly features blush prettily.~~\"But that was before,\" she shudders, \"the fire.\"")
                end
                add_answer("buy")
                if not local5 then
                    add_answer("here")
            ...
            elseif answer == "buy" then
                ...
                local8 = get_answer()
                if local8 then
                    add_dialogue("\"I am sorry, " .. local0 .. ",\" she giggles, \"but all we serve here are... spirits!\"*")
            ...
            elseif answer == "here" or answer == "tavern" then
                ...
                add_dialogue("\"Why, 'tis called the Keg O' Spirits. That's a fine name for a tavern, dost thou not agree?\" She smiles " .. local10 .. ".")
            ...
            elseif answer == "bye" then
                if not local1 then
                    add_dialogue("\"Goodbye, " .. local0 .. ".\" The pretty ghost turns away.*")
                else
