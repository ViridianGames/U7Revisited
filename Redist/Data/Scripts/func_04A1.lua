...
function func_04A1(eventid, itemref)
    ...
    if not get_flag(0x0203) then
        ...
        add_dialogue("Chad smiles. \"Hello, ", local3, ". I hope thy days are going well.\"")
    end
    ...
    while true do
        ...
        if answer == "name" then
            add_dialogue("\"Chad, at thy service, ", local1, ". And what is thy name?\"")
            remove_answer("name")
            local4 = call_090BH(local1, local2, local0)
            if local4 == local0 then
                add_dialogue("\"Greetings, ", local0, ". I am at thy service.\"")
                set_flag(0x01F1, true)
            elseif local4 == local1 then
                add_dialogue("\"Greetings, ", local1, ".\" He shrugs.")
            ...
            elseif local4 == local2 then
                ...
                if call_08F7H(-3) then
                    ...
                    local6 = call_08F7H(-1)
                    switch_talk_to(3, 0)
                    add_dialogue(local6 and "\"No, rogue! He is Iolo!\" He nods to Iolo. \"Thou... art a blind idiot!\"" or "\"No, rogue, thou art a blind idiot!\"")
                    _HideNPC(-3)
                    switch_talk_to(161, 0)
                end
