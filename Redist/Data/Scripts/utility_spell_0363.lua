...
function utility_spell_0363(eventid, objectref, arg1)
    local var_0000, var_0001, var_0002

    if eventid == 1 or eventid == 4 then
        var_0000 = object_select_modal() --- Guess: Selects target object
        var_0001 = utility_unknown_1069(get_spell_target(objectref), var_0000) --- Guess: Gets spell direction; External call to determine direction
        bark(objectref, "@Vas Flam Hur@") --- Guess: Item says spell incantation
        if not utility_unknown_1030() then --- External call to check spell success
