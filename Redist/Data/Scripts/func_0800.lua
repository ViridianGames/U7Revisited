...
function func_0800(eventid, itemref)
    ...
    if is_in_combat() then --- Guess: Checks combat state
        ...
        if calle_0801H(var_0000) then --- External call to check bedroll
            calle_0624H(var_0000) --- External call to unknown function
        end
        hide_npc(var_0001) --- Guess: Hides NPC
    else
        var_0003 = get_item_frame(var_0000) --- Guess: Gets item frame
        var_0004 = get_item_type(var_0000) --- Guess: Gets item type
        if var_0003 > 3 and not calle_0801H(var_0000) then
            var_0005 = unknown_0018H(var_0000) --- Guess: Gets position data
            var_0006 = unknown_0035H(0, 2, var_0004, var_0000) --- Guess: Sets NPC location
            -- Guess: sloop checks item positions
            for i = 1, 5 do
                var_0009 = {7, 8, 9, 6, 81}[i]
                if get_item_frame(var_0009) <= 2 then
        ...
        if not calle_0801H(itemref) then --- External call to check bedroll
            var_000B = add_container_items(356, {0, 30, 17492, 7715})
        end
        var_000C = remove_from_party(get_party_members(), 356) --- Guess: Removes from party
        unknown_001DH(14, 356) --- Guess: Sets object behavior
