...
function func_0947(eventid, itemref)
    ...
    if not var_0000 then
        ...
        for _, var_0005 in ipairs({3, 4, 5, 2}) do
            var_0001 = get_container_items(var_0005, 377, 0, 25, 356) --- Guess: Gets container items
        end
        var_0006 = 0
        var_0007 = {}
        for _, var_000A in ipairs({8, 9, 10, 1}) do
            if get_item_frame(var_000A) == 0 and check_item_flag(var_000A, 11) and compare_item_attribute(var_000A, -52) <= 25 then --- Guess: Checks item frame and flag
                var_0006 = var_0006 + 1
        ...
        if var_000B == 0 then
            ...
            var_000C = add_item_to_inventory(true, 359, 644, var_000B, 359) --- Guess: Adds item to inventory
            if var_000C then
                for _, var_000E in ipairs({13, 14, 10, 7}) do
                    if get_item_container(var_000E) then
                        set_item_flag(var_000E, 11, false) --- Guess: Sets item flag
                    else
                        calle_0925H(var_000E) --- External call to func_0925
                    end
                end
