...
function npc_brita_0168(eventid, objectref)
    ...
    if eventid == 1 then
        ...
        while true do
            ...
            elseif var_0003 == "Feridwyn" then
                if not get_flag(FLAG_GARRITT_PARENTS_SECRET) then
                    ...
                    var_0002 = npc_id_in_party(NPC_FERIDWYN)
                    if var_0002 then
                        switch_talk_to(NPC_FERIDWYN)
                        bark(NPC_FERIDWYN, "@Do not put stock in the proud boasts of wives, good Avatar. I am a simple man who only does what he can.@")
                        hide_npc(NPC_FERIDWYN)
                        switch_talk_to(NPC_BRITA)
                    end
                end
                remove_answer("Feridwyn")
            elseif var_0003 == "Fellowship" then
                if not get_flag(FLAG_JOIN_FELLOW) then
