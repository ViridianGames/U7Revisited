...
function func_0855(eventid, itemref)
    ...
    local9 = {1, 1, 1, 1, 10, 0}
    add_dialogue(itemref, "\"What dost thou want for thy refreshment?\"")
    while local1 do
        ...
        if local10 == 1 then
            ...
            local12 = 0
            add_dialogue(itemref, "^" .. local11 .. " Art thou still interested?\"")
            local13 = get_answer()
            if local13 then
            ...
            if local12 == 1 then
                add_dialogue(itemref, "\"It is thine!\"")
            elseif local12 == 2 then
                add_dialogue(itemref, "\"Thou cannot possibly carry that much!\"")
            elseif local12 == 3 then
                add_dialogue(itemref, "\"Thou dost not have enough gold, " .. local0 .. ".\"")
            end
            add_dialogue(itemref, "\"Wouldst thou care for something else?\"")
            local1 = get_answer()
        end
    end
    _RestoreAnswers()
