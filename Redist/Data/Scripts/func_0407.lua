    ...
    if eventid == 1 then
        ...
        if not answer then
            if not get_flag(26) then
                ...
                add_dialogue("\"How may I help thee, " .. local4 .. "?\" Sentri asks.")
            end
            answer = get_answer()
            return
        end
        ...
        elseif answer == "go home" then
            add_dialogue("\"Farewell, " .. local4 .. ". If thou dost need my services again, I shall be only too happy to comply.\"*")
            set_flag(219, false)
            switch_talk_to(7, 11)
            answers = {}
            answer = nil
